...
| --- | --- |
| [`business-wiki`](plugins/business-wiki) | A `business-docs/` tree — `wiki/` as the source of truth for business rules, with derived `index.tsv`, `rules/*.json`, and `openapi/` — plus four keeper agents that regenerate them and report drift, and search/outline/section tools so an agent reads the section it needs instead of the page. |
