...
#   2. No RF-N/AC-N header that existed in git HEAD has disappeared.
#
# (2) is a read of `git show`, and it degrades to a no-op outside a git repo or
# on a spec with no previous version. Nothing here writes to git — this is the
# only place in the plugin that mentions it at all.
#
